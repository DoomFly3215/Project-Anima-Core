...
public class MySQL {
    ...
    public Connection getConnection() throws SQLException {
        if (this.connection != null &&
                !this.connection.isClosed() && this.connection.isValid(100))
            return this.connection;
        try {
            this.connection = DriverManager.getConnection("jdbc:mysql://" + this.host + ":" + this.port + "/" + this.database + "?useUnicode=yes&characterEncoding=" + this.encoding + "&useSSL=" + (this.ssl ? "true" : "false") + "&autoReconnect=true", this.username, this.password);
            return this.connection;
        } catch (Exception e) {
